...
def _settings():
 ...
 return (p.getboolean('Timeline','AutoDetectIntro',fallback=True),
         max(1,p.getint('Timeline','IntroMaxEndSeconds',fallback=60)))

def _has_set_folders(shoot):
 return any(x.is_dir() and SET_RE.match(x.name.strip()) for x in shoot.iterdir())
...
def _fps(timeline):
 ...
 life.log('INTRO_FPS_FALLBACK',fps=25)
 return 25.0
...
def _detect_bounds(mp,timeline,max_seconds):
 ...
 temp_name=f'__DRPM_SCENE_{int(time.time()*1000)}'
 temp=timeline.DuplicateTimeline(temp_name)
 if temp is None:
  life.log('INTRO_NOT_DETECTED',reason='duplicate_failed');return None
 try:
  if not temp.DetectSceneCuts():
   life.log('INTRO_NOT_DETECTED',reason='detect_scene_cuts_failed');return None
  video=_items(temp,'video',1)
  if len(video)<3:
   life.log('INTRO_NOT_DETECTED',reason='fewer_than_two_cuts',segments=len(video));return None
  first_cut=int(round(float(video[1].GetStart(False))))
  second_cut=int(round(float(video[2].GetStart(False))))
  start=int(timeline.GetStartFrame() or 0);limit=start+int(round(_fps(timeline)*max_seconds))
  if second_cut>limit:
   life.log('INTRO_REJECTED',reason='second_cut_after_limit',first_cut=first_cut,second_cut=second_cut,limit=limit);return None
  if second_cut<=first_cut:
   life.log('INTRO_REJECTED',reason='invalid_cut_order',first_cut=first_cut,second_cut=second_cut);return None
  life.log('INTRO_DETECTED',first_cut=first_cut,second_cut=second_cut,max_seconds=max_seconds)
...
def _clip_info(item,record_start,record_end,track_index,media_type):
 item_start=int(round(float(item.GetStart(False))));source_start=int(item.GetSourceStartFrame())
 offset=record_start-item_start
...
def _rebuild(mp,timeline,bounds):
 intro_start,intro_end=bounds;original_name=timeline.GetName();tmp_name=f'__DRPM_REBUILD_{int(time.time()*1000)}'
 rebuilt=mp.CreateEmptyTimeline(tmp_name)
 if rebuilt is None:raise RuntimeError('Nelze vytvořit dočasnou timeline pro audio routing znělky.')
 try:
  video_tracks=int(timeline.GetTrackCount('video') or 0);audio_tracks=int(timeline.GetTrackCount('audio') or 0)
  ...
  while int(rebuilt.GetTrackCount('audio') or 0)<audio_tracks+1:
   subtype='stereo'
   try:
  ...
  for track in range(1,audio_tracks+1):
   for item in _items(timeline,'audio',track):
    ...
    overlap_start=max(start,intro_start);overlap_end=min(end,intro_end)
    if overlap_start<overlap_end:pieces.append((overlap_start,overlap_end,clean_track))
  ...
  if not mp.DeleteTimelines([timeline]):
   life.log('INTRO_BACKUP_TIMELINE_RETAINED',timeline=backup_name)
  life.log('INTRO_AUDIO_ROUTED',start_frame=intro_start,end_frame=intro_end,clean_track=clean_track)
  print(f'[OK] Znělka detekována mezi 1. a 2. střihem; audio -> A{clean_track} bez Voice Isolation')
  return rebuilt
 except Exception:
...
def configure(mp,timeline,shoot):
 ...
 if _has_set_folders(shoot):
  life.log('INTRO_SKIP',reason='set_folders_present');print('[OK] Detekce znělky přeskočena: SHOOTING obsahuje SET');return timeline
 bounds=_detect_bounds(mp,timeline,max_seconds)
 if not bounds:
  print('[OK] Použitelná znělka do první minuty nebyla detekována.');return timeline
 try:return _rebuild(mp,timeline,bounds)
 except Exception as e:
  life.log('INTRO_ROUTING_FAILED',error=str(e));print(f'[WARN] Znělka byla nalezena, ale bezpečné audio routování selhalo: {e}');return timeline
